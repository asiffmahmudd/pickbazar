...
const FilterDrawer = ({isFilterDrawerOpen, handleFilterDrawerClose, changeCategory}) => {
    ...
    return (
        <Drawer className="category-drawer drawer" anchor={"bottom"} open={isFilterDrawerOpen} onClose={() => handleFilterDrawerClose()}>
            ...
            <div className="mobile-view-filter row bg-white p-4">
                {
                    categories.map((category,index) => <FilterDrawerItem index={index} changeCategory={changeCategory} key={index} category={category}></FilterDrawerItem>)
                }
            </div>
        </Drawer>
    );
